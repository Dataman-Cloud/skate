            ...
            scope.$watch(attrs.carousel, function (products) {
                if (scope.products.length > 0) {
                    ...
                    for (var i = 0; i < products.length; i++) {
                        var productTitleLink = '/assets/img/posters/' + products[i].productId
                                               + '.png';
                        html += '<div class="item">' +
                                '<div class="thumbnail carousel-products">' +
                                '<a href="index.html#/products/' + products[i].productId
                                + '"><img alt="100%x180" src="' + productTitleLink + '"></a>' +
                                '</div>' +
                                '<span><a href="index.html#/products/' + products[i].productId
                                + '">' + products[i].name + '</a></span>' +
                                '</div>';
                    }
                                   ...
                                   function ($scope, $http) {
                                       ...
                                       var fetchCart = function () {
                                           $http({
                                               ...
                                               for (var i = 0; i < $scope.cart.lineItems.length;
                                                    i++) {
                                                   $scope.cart.lineItems[i].posterImage =
                                                       '/assets/img/posters/'
                                                       + $scope.cart.lineItems[i].product.productId
                                                       + '.png';
                                                   $scope.cart.lineItems[i].originalQuantity =
                                                       $scope.cart.lineItems[i].quantity;
                                                   $scope.cart.total +=
                                                       $scope.cart.lineItems[i].quantity
                                                       * $scope.cart.lineItems[i].product.unitPrice;
                                                   $scope.cart.totalItems +=
                                                       $scope.cart.lineItems[i].quantity;
                                               }
                                           }).error(function (data, status, headers, config) {
                                           });
    ...
    var res = {
        ...
        link: function (scope, element, attrs) {
            scope.$watch(attrs.carouselrelatedproducts, function (product) {
                if (scope.product != undefined ? scope.product.related != undefined
                        ? scope.product.related.length > 0 : false : false) {
                    product = scope.product;
                    var html = '';
                    for (var i = 0; i < product.related.length; i++) {
                        var relatedProductTitleLink = product.related[i].related.poster_image
                                                      || '/assets/img/posters/'
                                                      + product.related[i].related.title.replace(
                                '/', ' ') + '.jpg';
                        html += '<div class="item">' +
                                '<div class="thumbnail">' +
                                '<a href="index.html#/products/'
                                + product.related[i].related.title.replace('/', '%252F')
                                + '"><img src="' + relatedProductTitleLink + '"/></a>' +
                                '</div>' +
                                '<span><a href="index.html#/products/'
                                + product.related[i].related.title.replace('/', '%252F') + '">'
                                + product.related[i].related.title + '</a></span>' +
                                '</div>';
...
contentApp.controller('OrderListCtrl',
                      ['$scope', '$http', '$location', 'orderService', '$routeParams',
                       function ($scope, $http, $location, orderService, $routeParams) {
                           ...
                           var fetchAccounts = function () {
                               $http({
                                   ...
                                   $scope.orderListUrl =
                                       '/api/order/v1/accounts/' + defaultAccount.accountNumber
                                       + "/orders";
                                       ...
                                       $http({
                                           ...
                                           .success(function (orderData, status, headers, config) {
                                               ...
                                               for (var j = 0; j < $scope.orders.length; j++) {
                                                   ...
                                                   for (var i = 0;
                                                        i < $scope.orders[j].lineItems.length;
                                                        i++) {
                                                       $scope.orders[j].total +=
                                                           (($scope.orders[j].lineItems[i].price)
                                                            + ($scope.orders[j].lineItems[i].price
                                                               * $scope.orders[j].lineItems[i].tax))
...
contentApp.controller('OrderCtrl', ['$scope', '$http', '$location', 'orderService', '$routeParams',
                                    function ($scope, $http, $location, orderService,
                                              $routeParams) {
                                        $scope.orderItemUrl =
                                            '/api/order/v1/orders/' + $routeParams.orderId;
                                        var fetchOrder = function () {
                                            $http({
                                                ...
                                                .success(function (data, status, headers, config) {
                                                    ...
                                                    for (var i = 0;
                                                         i < $scope.order.lineItems.length; i++) {
                                                        $scope.order.total +=
                                                            (($scope.order.lineItems[i].price)
                                                             + ($scope.order.lineItems[i].price
                                                                * $scope.order.lineItems[i].tax))
...
contentApp.controller('CheckoutCtrl', ['$scope', '$http', '$location', 'orderService',
                                       function ($scope, $http, $location, orderService) {
...
contentApp.controller('AddToCartCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.addToCart = function () {
        var qty = $.trim($scope.qty);
        var alertObj = $("#addProductAlert");
        if(qty.length == 0){
            Alert(alertObj, "请输入添加商品数量!");
            return false;
        }
        qty = parseInt(qty);
        if (qty == 0) {
            Alert(alertObj, "添加商品不能为0!");
            return false;
        }
        if ($scope.inventoryNum < 0) {
            Alert(alertObj, "库存异常，请联系管理人!");
            return false;
        }
        if ($scope.inventoryNum < qty) {
            Alert(alertObj, "添加商品数多于库存总数，请重新输入!");
            return false;
        }
        ...
        if(qty > 0){
            ...
            $http(req).then(function () {
                $("#inventoryNum").text($scope.inventoryNum - qty);
                Alert(alertObj,"添加成功！");
            });
        }
    };
...
contentApp.controller('UpdateCartCtrl', ['$rootScope', '$scope', '$http', '$location',
                                         function ($rootScope, $scope, $http, $location) {
                                             $scope.productId = "";
                                             var selector = $("#updateProductAlert");
                                             $scope.updateCart = function () {
                                                 var delta = 0;
                                                 console.log($scope);
                                                 if ($scope.item.quantity >= 0
                                                     && $scope.item.originalQuantity > 0 &&
                                                     $scope.item.quantity
                                                     != $scope.item.originalQuantity) {
                                                     var updateCount = $scope.item.quantity
                                                                       - $scope.item.originalQuantity;
                                                     ...
                                                     if (delta >= 0) {
                                                         ...
                                                         $http(req).then(function () {
                                                             ...
                                                             if (updateCount <= 0) {
                                                                 $rootScope.$broadcast('cartEvents',
                                                                                       "update");
                                                             }
...
contentApp.controller('ProductItemCtrl', ['$scope', '$routeParams', '$http',
                                          function ($scope, $routeParams, $http) {
                                              $scope.productItemUrl =
                                                  '/api/catalog/v1/products/'
                                                  + $routeParams.productId;
                                              $scope.productsUrl = '/api/catalog/v1/catalog';
                                              $scope.products = [];
                                              $scope.productStockUrl =
                                                  '/api/inventory/v1/stock/'
                                                  + $routeParams.productId;
                                              $scope.productInventoryNumUrl =
                                                  '/api/inventory/v1/getInventoryNumByPid/'
                                                  + $routeParams.productId;
                                              ...
                                              var fetchProduct = function () {
                                                  ...
                                                  $http({
                                                      ...
                                                      function (data, status, headers, config) {
                                                          $scope.product = data;
                                                          $scope.product.poster_image =
                                                              '/assets/img/posters/'
                                                              + $scope.product.productId + '.png';
...
contentApp.controller('ChargeCtrl', ['$scope', '$http', '$location', 'orderService',
                                     function ($scope, $http, $location, orderService,
                                         ...
                                         $scope.chargeMoney = function () {
                                             $http({
                                                       method: 'GET',
                                                       url: $scope.chargeUrl + $scope.money + "/"
                                                            + $scope.count + "/user"
                                                   })
                                                 .success(function (data, status, headers, config) {
...
contentApp.controller('StockListCtrl',
                      ['$scope', '$http', '$location', 'orderService', '$routeParams',
                       function ($scope, $http, $location, orderService, $routeParams) {
                           $scope.stockUrl = '/api/inventory/v1/stock/getProductRelateStock';
                           $scope.stockUpdateUrl =
                               '/api/inventory/v1/stock/updateStockByProductId/';
                           ...
                           $scope.addToStock = function () {
                               $scope.oriNum = $scope.item.number;
                               var totalNum = parseInt($scope.oriNum) + parseInt($scope.addNum);
                               $http({
                                         method: 'GET',
                                         url: $scope.stockUpdateUrl + $scope.item.product.productId
                                              + '/' + totalNum
                                     }).success(function (data, status, headers, config) {
                                   var s = data;
