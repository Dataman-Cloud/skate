...
public class StockControllerV1 {
     ...
     * @return
     */
    @RequestMapping(path = "/getAll ", method = RequestMethod.GET, name = "getStockByProductId")
    ...
    public ResponseEntity<Stock> updateStock(@RequestBody Stock stock){
        return Optional.ofNullable(stockService.updateStock(stock)).map(result ->new ResponseEntity<>(result,HttpStatus.OK)).orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
